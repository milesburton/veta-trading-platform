    ...
    <div data-testid="component-picker" className="relative">
      ...
      {open.value && (
        <>
          ...
          <div className="absolute left-0 top-8 z-20 w-60 bg-surface border border-divider rounded shadow-xl text-xs">
            <div className="px-3 py-2 border-b border-divider font-semibold text-default uppercase tracking-wider text-[10px]">
              Add Panel
            </div>
            <ul className="py-1">
                  ...
                  <li key={id}>
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 transition-colors text-left gap-3 ${
                        disabled ? "opacity-40 cursor-not-allowed" : "hover:bg-panel cursor-pointer"
                      }`}
                    >
                      <div className="flex flex-col min-w-0">
                        <span
                          className={`font-medium ${disabled ? "text-muted" : "text-secondary"}`}
                        >
                          {PANEL_TITLES[id]}
                          {id === "admin" && (
                      ...
                      {!disabled && (
                        <span className="text-muted shrink-0 text-[10px]" title="Drag to place">
                          ⠿
                        </span>
                      )}
                    </button>
