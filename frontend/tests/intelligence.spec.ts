import { expect, traderTest } from "./helpers/fixtures.ts";
import type { AppPage } from "./helpers/pages/AppPage.ts";
...
const AAPL_SIGNAL = {
  ...
  factors: [
    ...
    { name: "relativeVolume", weight: 0.1, contribution: 0.05 },
    { name: "realisedVol", weight: -0.15, contribution: -0.05 },
    { name: "sectorRelativeStrength", weight: 0.2, contribution: 0.15 },
    { name: "eventScore", weight: 0.1, contribution: 0.05 },
    { name: "newsVelocity", weight: 0.1, contribution: 0.08 },
    { name: "sentimentDelta", weight: 0.1, contribution: 0.08 },
  ],
  ts: Date.now(),
};
...
traderTest.describe("Research Radar Panel", () => {
  traderTest(
    "signal bubbles appear when signalUpdate events are received",
    async ({ app, gateway, page }) => {
  ...
  traderTest(
    "ranked table shows symbol and direction for each signal",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "second signal for same symbol replaces the first (latest wins)",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "multiple symbols appear independently in the radar",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
...
traderTest.describe("Instrument Analysis Panel", () => {
  traderTest(
    "feature bars render when featureUpdate received for selected symbol",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "signal score value appears in the panel once signal is received",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
...
traderTest.describe("Signal Explainability Panel", () => {
  traderTest(
    "factor contribution bars and labels render for received signal",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "final score and confidence percentage display correctly",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "weight signs match: realisedVol weight is negative (shown as -)",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
...
traderTest.describe("AI Advisory Panel", () => {
  traderTest(
    "shows 'Not requested' status and Get Advisory button by default",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "shows error message when LLM service is disabled (503)",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
      ...
      await expect(
        panel.getByText(/failed to request advisory|llm service may not be enabled/i)
      ).toBeVisible({
        timeout: 5_000,
      });
    }
  ...
  traderTest(
    "shows requesting state while advisory POST is in flight",
    async ({ app, gateway, page }) => {
      ...
      await expect(panel.getByRole("button", { name: /requesting/i })).toBeVisible({
        timeout: 5_000,
      });
      resolveRequest();
    }
  ...
  traderTest(
    "renders advisory note content when advisoryUpdate WS event is received",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
  ...
  traderTest(
    "disclaimer text always visible regardless of advisory state",
    async ({ app, gateway, page }) => {
      await openResearchLayout(app);
