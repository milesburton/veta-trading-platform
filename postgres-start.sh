...
if [ ! -f "${PGDATA}/PG_VERSION" ]; then
  ...
  # Use trust auth during init so no password is required for the superuser
  su -s /bin/sh postgres -c "${PG_BIN}/initdb -D ${PGDATA} --auth=trust --username=postgres"

  # Allow network connections with md5 auth (appended after trust line so local stays trusted)
  echo "host all all 0.0.0.0/0 md5" >> "${PGDATA}/pg_hba.conf"
  echo "listen_addresses = '*'" >> "${PGDATA}/postgresql.conf"

  # Start temporarily to create app user/db, then stop
