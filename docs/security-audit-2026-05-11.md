...
In scope: the public attack surface of the deployment URL — the edge
Traefik (`edge/`), the tunnelled server Traefik (`traefik.yml`,
...
(adversary row 1) now able to hit every path the server Traefik routes,
...
- **Recommended fix:** In `compose.prod.yml` set `OAUTH2_SHARED_SECRET` from a server `.env` value that is generated per deployment (32+ random chars). Treat the fallback as fatal in production: in `user-service.ts:96`, refuse to start when `OAUTH2_SHARED_SECRET` equals `veta-dev-passcode` and `VETA_ALLOW_DEMO_PASSCODE` is not explicitly set. Rebuild the frontend bundle with `VITE_DEMO_PASSCODE` empty so the demo-fill becomes a no-op.
...
- **Recommended fix:** In `gateway.ts:71-75`, replace `"*"` with an exact allowlist of origins (the deployment URL, and optionally `http://localhost:5173` for dev). Drop `Authorization` from `Access-Control-Allow-Headers` since auth is cookie-only. Mirror the change in `user-service.ts:118-122`.
...
- **Why it matters publicly:** The site is now reachable over a real HTTPS origin where browsers honour `Secure`. Without it, a single mixed-content path, accidental :80 listener on the server Traefik, or local-network HTTP redirect can leak the session cookie. `SameSite=Lax` permits the cookie on top-level cross-site navigation, which is enough for CSRF on state-changing GETs and for a malicious site to drive a victim into a logged-in state on the deployment URL.
...
- **What:** The server Traefik runs with `api.dashboard: true` and `api.insecure: true`, exposing the dashboard on the container port 8080 with no auth.
- **Why it matters publicly:** The secure tunnel only forwards `443`, so the dashboard is not reachable through the deployment URL. However, if a docker-compose label or port mapping ever publishes 8080 to the server LAN (or worse, the host), the entire route table — including any auth tokens injected via headers — becomes browsable. The risk is one config change away.
- **Recommended fix:** Set `api.insecure: false` in `traefik.yml:8`, or add `api.dashboard: false` for the server deployment. If the dashboard is genuinely useful, expose it behind a `basicAuth` middleware on a non-public hostname only.
...
- **Why it matters publicly:** This is reconnaissance gold. An attacker scanning `/ready` gets a full inventory of running services and infers what code paths exist (e.g. `fixArchive`, `darkPool`, `replay`). It also makes a quick liveness oracle for a denial-of-service campaign.
...
- **Why it matters publicly:** Minimal — these strings are inert unless the user clicks a "services" panel link. But they advertise the dev port layout to anyone reading the bundle, helping an attacker fingerprint internal services if they ever land on the server LAN.
...
- **Recommended fix:** Add an `Origin` / `Referer` header check at the top of `requireAuth` in `gateway.ts:102` — reject non-deployment-URL origins on state-changing methods. Document the dependency in the threat-model so the next person changing SameSite knows what they're disabling.
...
- **What:** The edge Traefik proxies to `https://127.0.0.1:18443` with `serversTransport.insecureSkipVerify: true` because the server Traefik presents a self-signed cert. The secure tunnel provides transport security, so cert verification at this hop is redundant.
...
- **Recommended fix:** Keep as-is. Add a comment in `edge/traefik.yml:30-31`: `# insecureSkipVerify is required because the destination is 127.0.0.1:18443 reached via a secure tunnel from the server. Transport security is provided by the tunnel; do not "fix" this without redesigning the tunnel.`
...
- **Grafana / LGTM exposure under `/grafana`.** A public Grafana sub-path is configured. The labels in `observability/docker-compose.lgtm.yml` were not read in this audit and should be reviewed for the same auth questions as the Redpanda console (F-2).
