...
The gateway runs as a single Docker container on the server. It is deployed
...
- [Edge architecture](/veta-trading-platform/platform/edge-architecture/) — how the gateway fits into the
  overall platform topology
- [Service map](/veta-trading-platform/platform/services/) — the gateway's position in the service mesh
- [Risk architecture](/veta-trading-platform/platform/risk-architecture/) — how the gateway integrates with
  pre-trade risk checks
- [Archive service](/veta-trading-platform/platform/supporting-services/) — the archive service that persists
  all FIX messages
