...
- [Edge architecture](./edge-architecture) — how the gateway fits into the
  overall platform topology
- [Service map](./services) — the gateway's position in the service mesh
- [Risk architecture](./risk-architecture) — how the gateway integrates with
  pre-trade risk checks
- [Archive service](./supporting-services) — the archive service that persists
  all FIX messages
