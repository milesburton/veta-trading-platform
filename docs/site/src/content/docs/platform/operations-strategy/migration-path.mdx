...
2. **Network constraint.** Swarm overlays are scope=`swarm`; the existing `veta_trading-net` is a bridge (scope=`local`) shared with two off-repo compose projects (caddy + lgtm-grafana). A swarm stack cannot attach to a bridge regardless of `external: true`. Resolving this requires coordinated changes to the off-repo projects, meaningful work for benefits we do not yet need.
