...
The synthetic probe is a small Deno binary that pretends to be a user every
60 seconds. It's the source of truth for "is `veta.mnetcs.com` actually up
...
internet
   │
   ▼ HTTPS
ovh.agileview.co.uk (the OVH edge box)
   │
   ├──► veta-synthetic-probe.timer (every 60s)
   │      └─► /opt/veta-probe/probe.ts
   │            ├─ GET https://veta.mnetcs.com/
   │            ├─ POST .../oauth/guest
   │            └─ GET  .../api/gateway/ready
   │
   └──► Traefik :443 (this is what real users hit)
            │
            ▼ reverse SSH tunnel
        homelab Traefik :443 → frontend / gateway / etc.
```

Critically the probe runs on the **same machine that handles real traffic**,
so its `fetch()` call traverses the same Traefik → tunnel → homelab path
real users do. A probe on the homelab itself would always think the
platform was fine because it'd skip the tunnel hop.
...
so the probe doesn't need long-lived credentials. If the platform's guest
mode is disabled (`PUBLIC_GUEST_TRADING=false`) step 2 returns 403 and the
probe correctly fails — that's a meaningful signal (someone disabled the
public path).
...
These are queued for v2 and are deliberately not in v1: the simpler the
...
3. Fails the workflow on persistent failure → the standard GH email goes
   out.
...
disagree, the divergence itself is signal — e.g. an OVH-local network
issue would show up on the OVH probe only, while a TLS / DNS issue would
show up everywhere.
...
- **v2 checks**: WS frame count, order submit + cancel, market-data
  ...
  do them one at a time after v1 has a track record.
