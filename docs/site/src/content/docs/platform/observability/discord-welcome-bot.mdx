...
- **Container shows `healthy` in `docker ps` but `Raise.`/@mention messages get no reply and no log lines appear at all.** The Docker healthcheck only proves the HTTP port is serving `/health`; it does not prove the Discord Gateway websocket is currently connected. The reconnect loop can occasionally leave the socket dead without the container's own health probe noticing. Don't trust `docker ps` alone here: check `curl -sf http://localhost:5034/health` for `connected: true`, and check log recency (`docker logs veta-discord-bot-1 --since 1h`) for a `gateway ready` line within the last hour or so, that gap is the real liveness signal. If the connection is stale, `docker restart` the service to force a fresh Gateway handshake.
