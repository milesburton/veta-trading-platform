...
The <Svc name="journal" /> service exposes `GET /metrics/latency` returning per-stage `p50` / `p95` / `p99` for the order pipeline (`gateway`, `bus`, `oms`, `routedToChild`, `emsAck`, `filled`). Recent baselines and how they were measured live on the [Performance page](../../../analytics/performance/).
