...
flowchart LR
  ...
  M -.optional.-> P[POST /api/gateway/ticket-attachments/presign]
  P --> UP[Browser uploads directly to MinIO]
  UP -.-> M
  M --> G[POST /api/gateway/bug-report]
  G --> N[notifyDiscordBug]
  G --> T[createTicketForUserReport]
  N --> D[(Discord tickets channel)]
  T --> GH[(GitHub issue, attachments embedded)]
...
- Up to 5 attachments (screenshots or files), if any were added
...
Users can attach screenshots and files to a ticket:

- **Attach file**: any image or video file, up to 10MB each, up to 5 per ticket.
- **Capture screenshot**: uses the browser's Screen Capture API to grab a single frame of whatever the user shares. The share is stopped immediately after the frame is captured, it is not a recording.

Attachments upload directly from the browser to a MinIO bucket via a short-lived, gateway-issued presigned URL, they never pass through the gateway itself. The bucket is deliberately public-read (unauthenticated GET) because GitHub needs to fetch the image to render it inline in the issue body; object keys are UUID-based as the only protection against enumeration. In the created GitHub issue, image attachments render inline as markdown images, and other file types render as plain links.

Video *recording* (as opposed to a single captured frame) is not supported yet.

Requires `MINIO_ENDPOINT`, `MINIO_PUBLIC_URL`, `MINIO_ROOT_USER`, and `MINIO_ROOT_PASSWORD` to be configured on the gateway (see [Configuration](#configuration)). If MinIO isn't configured, the attach/capture buttons still appear but any attempt to upload fails with an inline error, the rest of the ticket form still works normally.
...
Attachments require MinIO to be configured:

| Env var | Purpose |
| --- | --- |
| `MINIO_ENDPOINT` | Internal address the gateway uses to sign presigned upload URLs, e.g. `http://minio:9000`. |
| `MINIO_PUBLIC_URL` | The browser- and GitHub-reachable base URL for the same bucket, e.g. `https://veta.example/attachments`. |
| `MINIO_ROOT_USER` / `MINIO_ROOT_PASSWORD` | Credentials the gateway signs presigned URLs with. |
...
5. Optionally attach a file or capture a screenshot.
6. Submit. If GitHub ticketing is configured, the success state links to the created issue.
