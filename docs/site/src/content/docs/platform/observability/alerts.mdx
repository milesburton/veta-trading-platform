...
Once a day at 09:00 UTC (override via `DISCORD_DAILY_SUMMARY_HOUR_UTC`), the gateway posts a rollup of the last 24 hours to the same Discord webhook:

```
✅ **VETA daily summary** · `a1b2c3d` (prod) · gateway uptime 2d 14h 3m

**Services (last 24h):** mean 99.8% up · worst window 96.9%
Now: all 32 services up ✅

**Alerts (last 24h):** 7 total · critical: 1, warning: 6
Last critical: 47m ago — `kill-switch` admin-triggered halt for AAPL
...
Header emoji follows the worst service-availability window in the day: ✅ 100%, ⚠️ at-or-above 95% but below 100%, 🚨 below 95%. So a daily summary with ✅ means every 5-second sample saw every non-optional service up.

Implementation in [`backend/src/gateway/daily-summary.ts`](https://github.com/milesburton/veta-trading-platform/blob/main/backend/src/gateway/daily-summary.ts), backed by the in-memory ring buffer in [`platform-stats.ts`](https://github.com/milesburton/veta-trading-platform/blob/main/backend/src/gateway/platform-stats.ts). Events older than 24h are pruned on every read.

The previous per-minute heartbeat (#319) was retired in #330 — 1440 messages/day buried real alerts under positive-liveness spam. The daily summary captures the same "is the platform alive" signal without the noise; real problems still arrive in real-time as severity-CRITICAL/WARNING alerts.
