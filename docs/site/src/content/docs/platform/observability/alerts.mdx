...
### Algo heartbeats are not a liveness signal

`algo.heartbeat` Kafka messages are order-lifecycle events (an algo strategy publishes one when it starts, slices, or completes an order), not a periodic keepalive. An earlier version of `gatewayMiddleware` treated any gap of more than 30 seconds between heartbeats for a given algo as a fault and raised a WARNING alert when the gap closed. On a paper-trading platform this fired constantly and for no real reason: an algo with no order flow for a while is routine, not broken, so every quiet period followed by a new order produced a false "heartbeat gap detected" alert, forwarded straight to the Discord channel.

That gap-detection logic has been removed. `onAlgoHeartbeat` now only marks the algo feed as alive (`feedReceived("algo")`); it never raises an alert. If genuine algo-liveness monitoring is wanted later, it needs a real periodic heartbeat published on a fixed interval independent of order flow, not order events repurposed as one. Do not reintroduce gap-based alerting on `algo.heartbeat` without that change.
...
3. SSH to the server and update `/opt/stacks/veta/.env`
...
Every rule has a `promtool` unit test in [`observability/prometheus-rules.test.yml`](https://github.com/milesburton/veta-trading-platform/blob/main/observability/prometheus-rules.test.yml). Tests cover the positive case (rule fires when threshold is breached for the configured `for:` window) and the locked-in negatives (no false positives below the threshold, market.* topics excluded from the topic-rate rules, per-group fan-out for consumer lag). Run them locally:

```bash
promtool test rules observability/prometheus-rules.test.yml
```

Or via docker if promtool is not installed:

```bash
docker run --rm \
  -v "$PWD/observability/prometheus-rules.yml":/prometheus-rules.yml \
  -v "$PWD/observability/prometheus-rules.test.yml":/test.yml \
  --entrypoint promtool prom/prometheus:v3.1.0 test rules /test.yml
```

Pre-commit runs these automatically when either file is staged. CI runs them on every PR that touches either file.
