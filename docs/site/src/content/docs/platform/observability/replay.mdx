...
| `DELETE` | `/sessions/{id}` | Delete session metadata (does **not** delete chunks) |
...
| Session deletion | Only `admin` and `compliance` users see the **Delete** button; it removes session metadata but not stored chunks (chunks are retained for compliance) |
