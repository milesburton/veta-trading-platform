...
```bash
ssh <user>@<server-host>.lan 'docker exec veta-gateway-1 sh -c \
...
Tokens have an expiry. Renew before they lapse: GitHub → Settings → Developer settings → Fine-grained tokens → regenerate → update `.env` on the server → `docker compose up -d gateway`. The old token becomes inert immediately.
</Aside>
