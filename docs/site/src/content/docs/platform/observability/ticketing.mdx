...
The token is a bearer credential. Inject via the homelab's `.env`:
...
```bash
ssh <user>@<homelab-host>.lan 'docker exec veta-gateway-1 sh -c \
...
Tokens have an expiry. Renew before they lapse: GitHub → Settings → Developer settings → Fine-grained tokens → regenerate → update `.env` on the homelab → `docker compose up -d gateway`. The old token becomes inert immediately.
</Aside>
