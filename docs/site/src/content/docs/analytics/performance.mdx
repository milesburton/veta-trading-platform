...
import LoadTestChart from "@docs/components/LoadTestChart.astro";
import scaling from "@docs/data/loadtest/2026-05-03-scaling.json";
...
1. **Pipeline latency is invariant under scaling.** Going from 100 to 500 concurrent orders moved end-to-end p99 from 291 ms to 283 ms, a 3% _decrease_ (within noise). If the algo's per-tick iteration over the active-order set were the bottleneck, p99 would have grown roughly linearly with order count. It did not.
...
- **Concurrency above 500.** The flat-latency observation is empirically true to 500 orders. The break point, where iteration _does_ start to matter, is somewhere above 500 and below the theoretical 50,000+ where microbenchmarks suggest it would. We have not bracketed this.
...
Live percentiles stream to Prometheus and are visualised on the **k6 Prometheus** dashboard in the local Grafana instance (port 3000 in the dev container; in production the same dashboard is reachable at `https://veta.example.com/grafana/`) for the duration of the run. Summary JSON and CSV are written to `docs/site/src/data/loadtest/<date>.json` and `<date>.csv`.
