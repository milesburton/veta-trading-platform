...
- **Checkbox column**: click the header checkbox to select all, or click individual checkboxes per row.
...
- Use the **Filter Bar** below the toolbar to build filter expressions (AND/OR logic with 13 operators).
...
- Bold any order with quantity above 10,000.
