...
| Core trading flows | 9 core services, all 9 algo strategies | 41 |
| Peripheral services | 4 core services, 12 peripheral services | 18 |
...
- **CI** (`test:testcontainers` task): runs on every push and PR, on the `integration` job's self-hosted homelab runner (see [CI/CD Pipeline](/development/ci-cd/)). `scripts/run-testcontainers.sh` sets `RUN_TESTCONTAINERS=1` and brings up an ephemeral Postgres + Redpanda per Deno.test block. That runner also carries the always-on production VETA stack's load, so `smoke.full.tc.test.ts` scales its timeouts via `TC_TIMEOUT_SCALE=3` there; see [Testcontainers integration tests](/development/testing/testcontainers/#tc_timeout_scale).
