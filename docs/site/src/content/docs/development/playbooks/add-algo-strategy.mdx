...
- Familiarity with the [Algo strategies](../../platform/algos/) page
...
5. **Document** in the strategy file's leading JSDoc. The first line becomes the title and the body becomes the description on [Algo strategies](../../platform/algos/) — picked up automatically by `docs/site/scripts/generate-algos.mjs` on the next docs build.
...
- **Hardcoding venue selection** — child orders should let the EMS choose the venue (see [Smart Order Router](../../platform/smart-order-router/)). Only set `venue` explicitly if your algo is doing best-price routing like SNIPER.
