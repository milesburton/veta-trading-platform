...
A provisioned Grafana panel on `https://veta.example.com/grafana/` shows "No data" or a partial render and you need to know why.
...
2. **Verify the source exists**:
   ...
   - **Loki stream**: `curl -G --data-urlencode 'query={<labels>}' http://<server-host>:3100/loki/api/v1/query | jq` — `result: []` means no log lines match those labels.
