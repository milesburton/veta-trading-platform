...
- SSH to the homelab (`ssh miles@192.168.1.245`)
...
2. **Verify the source exists**:
   - **Prometheus metric**: `curl -sf http://192.168.1.245:9090/api/v1/series?match[]=<metric_name>` — empty array means no series.
   - **Loki stream**: `curl -G --data-urlencode 'query={<labels>}' http://192.168.1.245:3100/loki/api/v1/query | jq` — `result: []` means no log lines match those labels.
...
4. **Verify the scrape works**:
   - `curl http://192.168.1.245:9090/api/v1/targets` — every target should show `health=up`.
...
6. **Service-down sanity check**: if multiple panels are empty, the underlying service may be down. `ssh miles@192.168.1.245 'supervisorctl status'` — but on docker-compose deployments, `docker compose ps` is the equivalent.
