...
- SSH to the homelab (`ssh <user>@<homelab-host>`)
...
2. **Verify the source exists**:
   - **Prometheus metric**: `curl -sf http://<homelab-host>:9090/api/v1/series?match[]=<metric_name>` — empty array means no series.
   - **Loki stream**: `curl -G --data-urlencode 'query={<labels>}' http://<homelab-host>:3100/loki/api/v1/query | jq` — `result: []` means no log lines match those labels.
...
4. **Verify the scrape works**:
   - `curl http://<homelab-host>:9090/api/v1/targets` — every target should show `health=up`.
...
6. **Service-down sanity check**: if multiple panels are empty, the underlying service may be down. `ssh <user>@<homelab-host> 'supervisorctl status'` — but on docker-compose deployments, `docker compose ps` is the equivalent.
