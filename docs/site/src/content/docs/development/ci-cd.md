...
- `Docs build & lint`
- `Deploy gate (compose + smoke)`
- `PR visual anomalies`

`Integration tests` is intentionally **not** in this list: the homelab self-hosted runner it depends on is shared with the always-on production stack (see [When it really is us](#when-it-really-is-us) above), so gating merges on it would make the whole repo unable to merge whenever that box is under load. It still runs and reports on every PR as a visible signal, it just doesn't block. Other path-conditional jobs (`Build base image`, `Capture UI screenshots`, `PR screenshot diff`, the per-service `Build *` matrix, the `Publish *` jobs) are excluded for the same underlying reason — they're either main-only or path-gated, and requiring a skipped check would deadlock merges.
