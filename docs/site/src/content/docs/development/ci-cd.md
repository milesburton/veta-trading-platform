...
- `Playwright UI tests (1/2)`
- `Playwright UI tests (2/2)`
- `Docs build & lint`
- `PR visual anomalies`

`Integration tests` is intentionally **not** in this list: the homelab self-hosted runner it depends on is shared with the always-on production stack (see [When it really is us](#when-it-really-is-us) above), so gating merges on it would make the whole repo unable to merge whenever that box is under load. It still runs and reports on every PR as a visible signal, it just doesn't block. `Deploy gate (compose + smoke)` is excluded for a different reason: it's path-conditional on `needs.changes.outputs.docker`, so it skips outright on any PR that doesn't touch Docker-relevant paths (docs-only changes, for example), and a required check that's routinely `skipping` deadlocks those PRs' merges. Its real gating happens through `publish-latest`'s own `needs: [docker-base, docker-services, deploy-gate, integration]` dependency, which only matters for the PRs where deploy-gate actually runs. Other path-conditional jobs (`Build base image`, `Capture UI screenshots`, `PR screenshot diff`, the per-service `Build *` matrix, the `Publish *` jobs) are excluded for the same underlying reason.

Note that GitHub's native PR auto-merge (the `Merge when ready` toggle, distinct from this repo's own `trusted-automerge` workflow) waits for **every** check suite to reach a terminal state, required or not. A long-running or hung non-required job like `Integration tests` can still delay a native-auto-merge PR even though it isn't blocking the actual merge gate. If a PR needs to land immediately and every required check above is green, a plain `gh pr merge --squash` (no `--admin`) succeeds despite a pending or failing non-required check.
