...
- **Backend**: {testInventory.backend.unit.cases} tests across {testInventory.backend.unit.files} files via `deno task test`. Pure function tests for OMS validation, algo slicing, FIX parsing, analytics math, grid filtering.
