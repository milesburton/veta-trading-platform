...
Only `trader` accounts may submit orders. `desk-head` and `risk-manager` are read-only oversight roles — see _Trading Styles & Desks_ below. All other non-trading roles are read-only or administrative.
...
_For architecture diagrams see [architecture.md](architecture.md). For deployment setup see [deployment.md](deployment.md). For individual service API docs see [api/](api/)._
