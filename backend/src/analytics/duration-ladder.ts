    ...
    for (let i = 1; i <= totalPeriods; i++) {
      ...
      for (const [label, pvAmt] of pvAttrib) {
        ...
        const fraction = priceResult.price * Math.abs(quantity) > 0
          ...
          : 0;
        const bucketDv01 = cfDv01 * fraction * priceResult.price * Math.abs(quantity) / (pv > 0 ? pv : 1);
        bucketAttribution.set(label, (bucketAttribution.get(label) ?? 0) + bucketDv01 * (pv > 0 ? pv / (priceResult.price * Math.abs(quantity)) : 0));
      }
    }
