...
function attributeToBuckets(t: number, pv: number): Map<string, number> {
  const result = new Map<string, number>(KEY_RATE_BUCKETS.map((b) => [b.tenorLabel, 0]));
...
export function computeDurationLadder(positions: BondPosition[]): DurationLadderResponse {
  ...
  const netBucketDv01 = new Map<string, number>(KEY_RATE_BUCKETS.map((b) => [b.tenorLabel, 0]));

  const positionResults: DurationLadderResponse["positions"] = positions.map((pos, bondIndex) => {
    const periodsPerYear = pos.periodsPerYear ?? 2;
    const { faceValue, couponRate, totalPeriods, yieldAnnual, quantity } = pos;
    ...
    for (let i = 1; i <= totalPeriods; i++) {
      ...
      const cf = i === totalPeriods ? couponPerPeriod + faceValue : couponPerPeriod;
      ...
      for (const [label, pvAmt] of pvAttrib) {
        ...
        const fraction =
          priceResult.price * Math.abs(quantity) > 0
            ? pvAmt / (priceResult.price * Math.abs(quantity))
            : 0;
        const bucketDv01 =
          (cfDv01 * fraction * priceResult.price * Math.abs(quantity)) / (pv > 0 ? pv : 1);
        bucketAttribution.set(
          label,
          (bucketAttribution.get(label) ?? 0) +
            bucketDv01 * (pv > 0 ? pv / (priceResult.price * Math.abs(quantity)) : 0)
        );
      }
    }
    ...
    for (let i = 1; i <= totalPeriods; i++) {
      ...
      for (const [label, dv01Amt] of pvAttrib) {
        bucketDv01Direct.set(label, (bucketDv01Direct.get(label) ?? 0) + dv01Amt);
      }
    }
  ...
  const totalPortfolioDv01 = positionResults.reduce((sum, p) => sum + p.totalDv01, 0);
