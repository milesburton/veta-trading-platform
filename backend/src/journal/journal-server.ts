...
async function ingestTick(msg: { prices?: Record<string, number>; volumes?: Record<string, number> }) {
...
async function flushWriteQueue() {
  ...
  if (!pruneScheduled) {
    ...
    setTimeout(async () => {
      ...
      } catch { /* ignore */ } finally { c.release(); }
    }, 5_000);
  }
}
...
function ingest(topic: string, value: any) {
  ...
  const raw = (topic === "orders.child" || topic === "orders.filled") ? null : value;
  writeQueue.push([
    ...
    fields.filled_qty, fields.market_price, fields.market_impact_bps, fields.algo_params, raw,
  ]);
}
    ...
    try {
      ...
      for (const ex of executions) {
        ...
        ingest("orders.filled", {
          ...
          filledQty: ex.cumQty, avgFillPrice: ex.avgPx, remainingQty: ex.leavesQty,
          commissionUSD: ex.commission, ts: ex.ts, _reconciled: true,
        });
        reconciled++;
      }
    } finally { client.release(); }

    if (reconciled > 0) console.log(`[journal] Reconciled ${reconciled} missing fill(s) from fix-archive`);
  } catch (err) {
    console.warn("[journal] Fill reconciliation failed:", (err as Error).message);
  }
}
...
let journalRowCount = 0;
{
  ...
  if (client) {
    try {
      const { rows } = await client.queryArray("SELECT COUNT(*) FROM journal.events");
      journalRowCount = Number(rows[0]?.[0] ?? 0);
    } catch { /* fresh DB */ } finally { client.release(); }
  }
}
const journalGroupId = journalRowCount === 0
  ? `journal-fresh-${Date.now().toString(36)}` : "journal-group";
const marketGroupId = journalRowCount === 0
  ? `journal-market-fresh-${Date.now().toString(36)}` : "journal-market";
console.log(`[journal] Consumer groups: ${journalGroupId}, ${marketGroupId} (${journalRowCount} rows)`);

createConsumer(journalGroupId, CONSUME_TOPICS).then((consumer) => {
  consumer.onMessage((topic, value) => ingest(topic, value as Record<string, unknown>));
...
function rowToEntry(row: any[]) {
  ...
  return {
    ...
    user_id, algo, instrument, side, order_id, child_id,
    quantity, limit_price, fill_price, filled_qty, market_price, market_impact_bps,
    algo_params, raw: rawParsed,
  };
}
...
async function handle(req: Request): Promise<Response> {
  ...
  if (req.method === "GET" && path === "/journal") {
    ...
    if (orderId)    { conditions.push(`order_id = $${p++}`);   params.push(orderId); }
    if (algo)       { conditions.push(`algo = $${p++}`);       params.push(algo); }
    ...
    try {
      const { rows } = await client.queryArray(
        `SELECT id, event_id, event_type, ts, user_id, algo, instrument, side, order_id, child_id,
         ...
         FROM journal.events WHERE ${where} ORDER BY ts DESC LIMIT $${p} OFFSET $${p + 1}`,
        [...params, limit, offset],
      );
      const { rows: countRows } = await client.queryArray(
        `SELECT COUNT(*) FROM journal.events WHERE ${where}`, params,
      );
      return json({ total: Number(countRows[0][0]), limit, offset, entries: rows.map(rowToEntry) });
    } finally { client.release(); }
  }
  ...
  if (req.method === "POST" && path === "/grid/query") {
    ...
    try { body = await req.json() as GridQueryRequest; }
    catch { return json({ error: "Invalid JSON body" }, 400); }

    const { gridId, filterExpr, sortField, sortDir, offset = 0, limit = 200 } = body;
    if (!gridId || !filterExpr) return json({ error: "gridId and filterExpr are required" }, 400);
      ...
      gridId === "executions"
        ? allOrders.flatMap((o) =>
            (o.children as Record<string, unknown>[]).map((c) => ({
              ...c, parentId: o.id, asset: o.asset, strategy: o.strategy, userId: o.userId,
            }))
          )
        : allOrders;
    ...
    obsProducer?.send("grid.query", {
      gridId, total, evalMs, userId: req.headers.get("x-user-id") ?? null, ts: Date.now(),
    }).catch(() => {});
...
async function reconstructOrders(lookbackMs = GRID_LOOKBACK_MS): Promise<Record<string, unknown>[]> {
  const now = Date.now();
...
async function _reconstructOrders(lookbackMs: number): Promise<Record<string, unknown>[]> {
  ...
  try {
    ...
    for (const [
      orderId, eventType, ts, instrument, side, quantity, limitPrice,
      algo, userId, algoParams, raw,
    ] of structureRows as unknown[][]) {
      const tsMs = ts instanceof Date ? ts.getTime() : Number(ts);
      if (eventType === "orders.submitted" || (eventType === "orders.rejected" && !orderMap.has(orderId as string))) {
        ...
        orderMap.set(orderId as string, {
          ...
          expiresAt: eventType === "orders.submitted" ? (rawObj.expiresAt ?? tsMs + 86_400_000) : tsMs + 86_400_000,
          strategy: algo ?? rawObj.strategy ?? "LIMIT",
          status: eventType === "orders.rejected" ? "rejected" : "pending",
          rejectReason: eventType === "orders.rejected" ? (rawObj.reason ?? rawObj.message ?? null) : undefined,
          filled: 0,
    ...
    for (const [orderId, eventType, , side, quantity, limitPrice, filledQty, childId] of activityRows as unknown[][]) {
      const order = orderMap.get(orderId as string);
      if (!order) continue;
      if (eventType === "orders.filled") {
        ...
        (order.children as unknown[]).push({
          id: childId ?? "", side: side ?? order.side,
          quantity: quantity ?? 0, limitPrice: limitPrice ?? 0,
          filledQty: 0, avgFillPrice: 0, commissionUSD: 0, status: "pending",
        });
        if (order.status === "pending") order.status = "working";
      }
    }
