...
async function pruneOldEvents(): Promise<void> {
  ...
  try {
    ...
    logger.info(`Prune: removed ${result.rowCount ?? 0} events older than ${RETENTION_DAYS}d`);
  } catch (err) {
...
function ingest(topic: string, value: KafkaEventValue) {
  ...
  const ts = new Date(value.ts ?? arrivedAt.getTime());
...
// acceptable noise.
const journalGroupId =
  journalRowCount === 0 ? `journal-fresh-${Date.now().toString(36)}` : "journal-group";
const marketGroupId =
  journalRowCount === 0 ? `journal-market-fresh-${Date.now().toString(36)}` : "journal-market";
...
async function handle(req: Request): Promise<Response> {
  ...
  if (req.method === "GET" && path === "/orders") {
    ...
    const lookbackMs =
      Number.isFinite(lookbackMsParam) && lookbackMsParam > 0
...
async function reconstructOrders(
  ...
  if (
    _ordersCache &&
    _ordersCache.lookbackMs === lookbackMs &&
