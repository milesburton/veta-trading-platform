...
async function fetchFxRate(symbol: string): Promise<CachedQuote | null> {
  ...
  if (parts.length !== 2) {
    console.warn(`[alpha-vantage-fx] Invalid FX symbol format: ${symbol} (expected BASE/QUOTE)`);
    return null;
  }
  const [fromCurrency, toCurrency] = parts;
  try {
    console.log(`[alpha-vantage-fx] Polling CURRENCY_EXCHANGE_RATE for ${symbol}`);
    const url =
      `${ALPHA_VANTAGE_BASE}?function=CURRENCY_EXCHANGE_RATE&from_currency=${encodeURIComponent(fromCurrency)}&to_currency=${encodeURIComponent(toCurrency)}&apikey=${ALPHA_VANTAGE_KEY}`;
    const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json() as Record<string, Record<string, string>>;
    const rate = data["Realtime Currency Exchange Rate"];
    if (!rate || !rate["5. Exchange Rate"]) {
      console.warn(`[alpha-vantage-fx] No rate data for ${symbol} — rate limit or invalid pair?`);
      return null;
    }
    ...
    console.warn(`[alpha-vantage-fx] Rate fetch failed for ${symbol}: ${(err as Error).message}`);
    return null;
  }
}
