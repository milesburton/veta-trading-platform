...
async function fetchTiingoEquity(symbol: string): Promise<CachedQuote | null> {
  try {
    ...
    const url = `https://api.tiingo.com/iex/${symbol.toLowerCase()}?token=${TIINGO_KEY}`;
...
async function fetchTiingoFx(symbol: string): Promise<CachedQuote | null> {
  ...
  try {
    ...
    const url = `https://api.tiingo.com/tiingo/fx/${pair}/top?token=${TIINGO_KEY}`;
    const res = await fetch(url, { signal: AbortSignal.timeout(10_000) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
...
export const tiingoProvider: ProviderDef = {
  id: "tiingo",
  label: "Tiingo",
  description: "Real-world equity and FX quotes via Tiingo REST API. Free tier: daily data.",
  requiresApiKey: true,
