  ...
  async fn(t) {
    ...
    try {
      ...
      await t.step("GET /shared-workspaces returns 401 without auth", async () => {
        const res = await fetch(`${GW}/shared-workspaces`, { signal: T(5_000) });
      ...
      await t.step("OAuth2 register with invalid password returns 401", async () => {
  ...
  async fn(t) {
    ...
    try {
      await t.step("news GET /news?symbol=AAPL returns array", async () => {
        const res = await fetch(`${NEWS}/news?symbol=AAPL&limit=5`, { signal: T(5_000) });
        assertEquals(res.status, 200);
      ...
      await t.step("recommendation-engine GET /recommendations returns array", async () => {
        const res = await fetch(`${REC}/recommendations?limit=10`, { signal: T(5_000) });
        assertEquals(res.status, 200);
      ...
      await t.step("observability POST /events/batch accepts array and returns count", async () => {
