  ...
  fn: (t) =>
    withTimeout(async () => {
      ...
      try {
        ...
        await t.step("every service /health response includes a version field", async () => {
          const services = Object.keys(stack.urls) as (keyof TestStack["urls"])[];
          const missing = [];
    ...
    withTimeout(async () => {
      ...
      async function livePrice(token: string, symbol: string): Promise<number> {
        try {
          ...
          return (
            (assets as { symbol: string; price: number }[]).find((a) => a.symbol === symbol)
              ?.price ?? 190
          );
        } catch {
          return 190;
      ...
      try {
        ...
        await t.step("GET /shared-workspaces returns 401 without auth", async () => {
        ...
        await t.step("GET /advisory/admin/state returns 200 for admin", async () => {
        ...
        await t.step("market-sim WebSocket emits tick data within 20s", async () => {
        ...
        await t.step("OAuth2 token exchange sets veta_user cookie for alice", async () => {
          let res: Response | null = null;
          for (let attempt = 0; attempt < 3; attempt++) {
        ...
        await t.step("GET /orders returns array", async () => {
          const res = await fetch(`${J}/orders?limit=10`, { signal: T(5000) });
          assertEquals(res.status, 200);
        ...
        await t.step("fix-archive /health includes executions count", async () => {
          const res = await fetch(`${FX}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            status: string;
            executions: number;
        ...
        await t.step("LIMIT order reaches filled or expired within 90s", async () => {
          const token = await login(stack, "alice");
        ...
        await t.step("TWAP order reaches filled or expired within 90s", async () => {
          const token = await login(stack, "alice");
          const price = await livePrice(token, "AAPL");
          const { clientOrderId } = await submitOrder("alice", {
            asset: "AAPL",
        ...
        await t.step("SNIPER order reaches filled or expired within 60s", async () => {
          const token = await login(stack, "alice");
          const price = await livePrice(token, "AAPL");
          const { clientOrderId } = await submitOrder("alice", {
            asset: "AAPL",
        ...
        await t.step("OAuth2 authorize with invalid password returns 401", async () => {
          const res = await fetch(`${US}/oauth/authorize`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        await t.step("OAuth2 authorize with missing password returns 401", async () => {
          const res = await fetch(`${US}/oauth/authorize`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        await t.step("OAuth2 public registration is disabled by default (403)", async () => {
        ...
        await t.step(
          ...
          async () => {
            const res = await fetch(`${US}/personas`, { signal: T(5000) });
            assertEquals(res.status, 200);
            const body = (await res.json()) as {
              personas: Array<{
                id: string;
    ...
    withTimeout(async () => {
      ...
      try {
        ...
        await t.step(
          ...
          async () => {
            const res = await fetch(`${NEWS}/sources`, { signal: T(5000) });
            assertEquals(res.status, 200);
            const body = (await res.json()) as {
              id: string;
              label: string;
        ...
        await t.step("recommendation-engine /health includes count", async () => {
          const res = await fetch(`${REC}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
        ...
        await t.step("recommendation-engine GET /recommendations returns array", async () => {
        ...
        await t.step("market-data-service GET /sources returns array", async () => {
          const res = await fetch(`${MDS}/sources`, { signal: T(5000) });
          assertEquals(res.status, 200);
        ...
        await t.step("feature-engine /health includes trackedSymbols", async () => {
          const res = await fetch(`${FEATURE}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            status: string;
        ...
        await t.step("signal-engine /health includes trackedSymbols", async () => {
          const res = await fetch(`${SIGNAL}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            status: string;
            trackedSymbols: number;
        ...
        await t.step("scenario-engine /health is ok", async () => {
          const res = await fetch(`${SCENARIO}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
          assertEquals(((await res.json()) as { status: string }).status, "ok");
        });

        await t.step("llm-advisory /health reports status and trackedSymbols", async () => {
          const res = await fetch(`${LLM}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            status: string;
            trackedSymbols: number;
        ...
        await t.step(
          ...
          async () => {
            const res = await fetch(`${LLM}/admin/state`, { signal: T(5000) });
            assertEquals(res.status, 200);
            const body = (await res.json()) as {
              state: string;
              pendingJobs: number;
        ...
        await t.step("rfq-service GET /rfq/stats returns valid structure", async () => {
          const res = await fetch(`${RFQ}/rfq/stats`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            service: string;
            total: number;
        ...
        await t.step("dark-pool GET /pool/stats returns valid structure", async () => {
          const res = await fetch(`${DARK}/pool/stats`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            service: string;
        ...
        await t.step("ccp-service GET /ccp/stats returns valid structure", async () => {
          const res = await fetch(`${CCP}/ccp/stats`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as {
            service: string;
            totalNovated: number;
        ...
        await t.step("observability GET /health returns ok", async () => {
          const res = await fetch(`${OBS}/health`, { signal: T(5000) });
          assertEquals(res.status, 200);
        ...
        await t.step("replay GET /config returns recordingEnabled boolean", async () => {
          const res = await fetch(`${REPLAY}/config`, { signal: T(5000) });
          assertEquals(res.status, 200);
          const body = (await res.json()) as { recordingEnabled: boolean };
          assertEquals(typeof body.recordingEnabled, "boolean");
        });

        await t.step("replay session CRUD: create, list, end, events, delete", async () => {
          ...
          const chunkRes = await fetch(`${REPLAY}/sessions/${sessionId}/chunks`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              seq: 0,
          ...
          const deleteChunks = await fetch(`${REPLAY}/sessions/${sessionId}/chunks`, {
            method: "DELETE",
            signal: T(5000),
          });
