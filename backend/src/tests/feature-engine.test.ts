...
Deno.test("computeMomentum: positive, negative, and flat movement", () => {
  ...
  const up = [...base]; up[up.length - 1] = 110;
  assertAlmostEquals(computeMomentum(up), 0.1, 1e-6, "10% gain");

  const down = [...base]; down[down.length - 1] = 90;
...
Deno.test("computeRelativeVolume: 2× and 0.5× average", () => {
  ...
  const high = [...vols]; high[high.length - 1] = 2000;
  assertAlmostEquals(computeRelativeVolume(high), 2.0, 0.01, "2× volume");

  const low = [...vols]; low[low.length - 1] = 500;
...
Deno.test("computeSectorRelativeStrength: outperform, underperform, match sector", () => {
  const symbolUp10 = [...Array(20).fill(100)]; symbolUp10[19] = 110;
  const sectorUp5  = [...Array(20).fill(100)]; sectorUp5[19] = 105;

  assertAlmostEquals(computeSectorRelativeStrength(symbolUp10, [sectorUp5]), 0.05, 0.001, "+5% vs sector");
  assertAlmostEquals(computeSectorRelativeStrength(sectorUp5, [symbolUp10]), -0.05, 0.001, "-5% vs sector");
...
function makeNews(overrides: Partial<NewsEvent>): NewsEvent {
  return {
    id: "n1", source: "Yahoo", headline: "Test",
    tickers: ["AAPL"], sentiment: "neutral", sentimentScore: 0,
    relevanceScore: 0.5, publishedAt: Date.now() - 10_000, ts: Date.now() - 10_000,
    ...overrides,
  };
}
...
Deno.test("computeNewsVelocity: no news → 0; counts only recent items for this symbol", () => {
  ...
  const news = [
    ...
    makeNews({ id: "n3", tickers: ["MSFT"], ts: now - 5_000, publishedAt: now - 5_000 }), // different symbol
...
Deno.test("computeSentimentDelta: no news or single item → 0", () => {
  ...
  assertEquals(computeSentimentDelta("AAPL", [makeNews({ ts: Date.now() - 10_000, publishedAt: Date.now() - 10_000 })]), 0);
});
