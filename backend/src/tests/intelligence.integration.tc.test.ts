  ...
  async fn(t) {
    ...
    try {
      await t.step("feature-engine is healthy", async () => {
        const h = await getJson<{ status: string; service: string }>(
          `${FE}/health`,
        );
      ...
      await t.step(
        "signal-engine default weights satisfy invariants",
        async () => {
      ...
      await t.step(
        "PUT /weights round-trips a single weight change",
        async () => {
      ...
      await t.step("feature-engine returns all 7 fields for AAPL", async () => {
        ...
        for (
          ...
          assertEquals(
            typeof fv?.[f],
            "number",
            `Field ${f} should be a number`,
          );
        }
      });

      await t.step(
        "signal-engine returns a well-formed Signal for AAPL",
        async () => {
          ...
          assert(
            Array.isArray(signal?.factors) &&
              (signal?.factors as unknown[]).length === 7,
          );
        },
      );
      ...
      await t.step("market-data-adapters returns seeded events", async () => {
        const events = await getJson<
          Array<{ id: string; type: string; impact: string }>
        >(
          `${MA}/events?limit=20`,
        );
      ...
      await t.step(
        "gateway proxies GET /intelligence/weights with auth",
        async () => {
          ...
          assert(
            res.ok,
            `GET /intelligence/weights via gateway → ${res.status}`,
          );
