...
Deno.test({
  ...
  async fn(t) {
    ...
    try {
      await t.step("producer connects and reports ready within a few seconds", async () => {
        const producer = await messaging.createProducer(`prod-${crypto.randomUUID().slice(0, 6)}`);
      ...
      await t.step("invalid JSON message is silently skipped, doesn't crash consumer", async () => {
