...
Deno.test("[risk-checks] userGrossNotional sums absolute notional across symbols using mark price when available", () => {
  ...
  setPosition(state, "alice", {
    symbol: "AAPL", netQty: 100, avgPrice: 150, costBasis: 15_000, realisedPnl: 0, fillCount: 1,
  });
  setPosition(state, "alice", {
    symbol: "MSFT", netQty: -50, avgPrice: 350, costBasis: 17_500, realisedPnl: 0, fillCount: 1,
  });
...
Deno.test("[risk-checks] userGrossNotional falls back to avgPrice when mark unavailable", () => {
  const state = makeState();
  setPosition(state, "alice", {
    symbol: "ZZZZ", netQty: 100, avgPrice: 50, costBasis: 5_000, realisedPnl: 0, fillCount: 1,
  });
...
Deno.test("[risk-checks] userTotalPnl sums realised plus unrealised across positions", () => {
  ...
  setPosition(state, "alice", {
    symbol: "AAPL", netQty: 100, avgPrice: 150, costBasis: 15_000, realisedPnl: 200, fillCount: 1,
  });
...
Deno.test("[risk-checks] pruneRecent drops orders older than 5s", () => {
  const state = makeState({
    recentOrders: [
      { userId: "alice", symbol: "AAPL", side: "BUY", quantity: 1, limitPrice: 1, ts: 990_000 },
      { userId: "alice", symbol: "AAPL", side: "BUY", quantity: 1, limitPrice: 1, ts: 999_000 },
    ],
    now: () => 1_000_000,
  });
  pruneRecent(state);
...
Deno.test("[risk-checks] fat-finger: order at mid passes", () => {
  const state = makeState({ prices: { AAPL: 150 } });
  assertEquals(checkFatFingerPrice(state, makeRequest({ limitPrice: 150 })), null);
});

Deno.test("[risk-checks] fat-finger: 4% above mid passes when threshold is 5%", () => {
  const state = makeState({ prices: { AAPL: 100 } });
  assertEquals(checkFatFingerPrice(state, makeRequest({ limitPrice: 104 })), null);
});

Deno.test("[risk-checks] fat-finger: 6% above mid is rejected when threshold is 5%", () => {
  const state = makeState({ prices: { AAPL: 100 } });
  const hit = checkFatFingerPrice(state, makeRequest({ limitPrice: 106 }));
  assert(hit, "expected hit");
  assertEquals(hit!.code, "FAT_FINGER_PRICE");
  assert(hit!.message.includes("above"));
});

Deno.test("[risk-checks] fat-finger: 6% below mid says 'below'", () => {
  const state = makeState({ prices: { AAPL: 100 } });
  const hit = checkFatFingerPrice(state, makeRequest({ limitPrice: 94 }));
  assert(hit, "expected hit");
  assert(hit!.message.includes("below"));
});

Deno.test("[risk-checks] fat-finger: missing mid price skips check", () => {
  const state = makeState({ prices: {} });
  assertEquals(checkFatFingerPrice(state, makeRequest({ limitPrice: 1_000_000 })), null);
});

Deno.test("[risk-checks] fat-finger: zero mid price skips check", () => {
  const state = makeState({ prices: { AAPL: 0 } });
  assertEquals(checkFatFingerPrice(state, makeRequest({ limitPrice: 50 })), null);
});
...
Deno.test("[risk-checks] max open orders: under limit passes", () => {
  const state = makeState();
  state.activeOrderCounts.set("alice", 49);
  assertEquals(checkMaxOpenOrders(state, makeRequest()), null);
});

Deno.test("[risk-checks] max open orders: at limit is rejected (>= comparison)", () => {
  const state = makeState();
  state.activeOrderCounts.set("alice", 50);
  const hit = checkMaxOpenOrders(state, makeRequest());
  assert(hit);
  assertEquals(hit!.code, "MAX_OPEN_ORDERS");
});
...
Deno.test("[risk-checks] ADV: order at threshold % is allowed (strict >)", () => {
  const state = makeState({ volumes: { AAPL: 1_000 } });
  state.config.maxAdvPct = 5;
  assertEquals(checkOrderSizeVsAdv(state, makeRequest({ quantity: 50 })), null);
});

Deno.test("[risk-checks] ADV: order above threshold % is rejected", () => {
  const state = makeState({ volumes: { AAPL: 1_000 } });
  state.config.maxAdvPct = 5;
  const hit = checkOrderSizeVsAdv(state, makeRequest({ quantity: 51 }));
  assert(hit);
  assertEquals(hit!.code, "ORDER_SIZE_VS_ADV");
});

Deno.test("[risk-checks] ADV: missing or zero ADV skips check", () => {
  assertEquals(
    checkOrderSizeVsAdv(makeState({ volumes: {} }), makeRequest({ quantity: 1_000_000 })),
    null,
  );
  assertEquals(
    checkOrderSizeVsAdv(
      makeState({ volumes: { AAPL: 0 } }),
      makeRequest({ quantity: 1_000_000 }),
    ),
    null,
  );
});
...
Deno.test("[risk-checks] position notional: under limit allowed", () => {
  const state = makeState();
  state.config.maxGrossNotional = 100_000;
  assertEquals(
    checkPositionNotional(state, makeRequest({ quantity: 100, limitPrice: 50 })),
    null,
  );
});

Deno.test("[risk-checks] position notional: at limit allowed (strict >)", () => {
  const state = makeState();
  state.config.maxGrossNotional = 5_000;
  assertEquals(
    checkPositionNotional(state, makeRequest({ quantity: 100, limitPrice: 50 })),
    null,
  );
...
Deno.test("[risk-checks] position notional: over limit rejected", () => {
  const state = makeState();
  state.config.maxGrossNotional = 5_000;
  const hit = checkPositionNotional(state, makeRequest({ quantity: 101, limitPrice: 50 }));
  assert(hit);
  assertEquals(hit!.code, "POSITION_NOTIONAL_LIMIT");
});
...
Deno.test("[risk-checks] daily P&L stop: P&L at threshold rejected and breaker callback fires", () => {
  ...
  setPosition(state, "alice", {
    symbol: "AAPL", netQty: 100, avgPrice: 200, costBasis: 20_000, realisedPnl: 0, fillCount: 1,
  });
  state.config.maxDailyLoss = -10_000;
...
Deno.test("[risk-checks] concentration: single symbol exceeds pct rejected", () => {
  const state = makeState({ prices: { AAPL: 100 } });
  setPosition(state, "alice", {
    symbol: "AAPL", netQty: 100, avgPrice: 100, costBasis: 10_000, realisedPnl: 0, fillCount: 1,
  });
...
Deno.test("[risk-checks] concentration: diverse book passes", () => {
  ...
  setPosition(state, "alice", {
    symbol: "MSFT", netQty: 100, avgPrice: 100, costBasis: 10_000, realisedPnl: 0, fillCount: 1,
  });
  setPosition(state, "alice", {
    symbol: "NVDA", netQty: 100, avgPrice: 100, costBasis: 10_000, realisedPnl: 0, fillCount: 1,
  });
  state.config.maxConcentrationPct = 50;
  assertEquals(
...
Deno.test("[risk-checks] runChecks returns allowed=true with empty reasons when nothing fails", () => {
  ...
  setPosition(state, "alice", {
    symbol: "MSFT", netQty: 1_000, avgPrice: 200, costBasis: 200_000, realisedPnl: 0, fillCount: 1,
  });
  setPosition(state, "alice", {
    symbol: "NVDA", netQty: 500, avgPrice: 500, costBasis: 250_000, realisedPnl: 0, fillCount: 1,
  });
