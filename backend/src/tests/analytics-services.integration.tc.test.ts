...
Deno.test({
  ...
  async fn(t) {
    ...
    try {
      await t.step(
        "analytics POST /bond-price returns a coherent pricing result",
        async () => {
      ...
      await t.step(
        "analytics POST /bond-price prices a discount below a par bond",
        async () => {
      ...
      await t.step(
        "analytics POST /bond-price rejects missing fields",
        async () => {
          const res = await fetch(`${AN}/bond-price`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      await t.step(
        "analytics POST /yield-curve returns a populated curve",
        async () => {
          ...
          assert(
            ...
            "yield curve should have points",
          );
        },
      );
      ...
      await t.step("news GET /news returns an array for a symbol", async () => {
        ...
        assert(
          ...
          "news items should be an array",
        );
      });

      await t.step(
        "news GET /sources lists configured feed sources",
        async () => {
      ...
      await t.step(
        "recommendation-engine GET /recommendations returns an array",
        async () => {
