...
Deno.test({
  ...
  async fn(t) {
    ...
    try {
      await t.step("routes valid messages to the per-topic handler with typed payload", async () => {
        const orders: Order[] = [];
        const fills: Fill[] = [];
        const groupId = `typed-${crypto.randomUUID().slice(0, 6)}`;
        const consumer = await messaging.createTypedConsumer(groupId, [
          { topic: orderTopic, schema: OrderSchema, handler: (o: Order) => { orders.push(o); } },
          { topic: fillTopic, schema: FillSchema, handler: (f: Fill) => { fills.push(f); } },
        ]);
        ...
        while (Date.now() < ready && !prod.isReady()) await new Promise((r) => setTimeout(r, 100));
      ...
      await t.step("drops messages that fail schema validation; calls onInvalid", async () => {
        ...
        const consumer = await messaging.createTypedConsumer(groupId, [
          ...
          onInvalid: (topic: string, raw: unknown) => { invalid.push({ topic, raw }); },
        });
        await new Promise((r) => setTimeout(r, 3_000));
      ...
      await t.step("uses default onInvalid (logs + drops) when none provided", async () => {
        ...
        while (Date.now() < deadline && orders.length < 1) await new Promise((r) => setTimeout(r, 100));
        assertEquals(orders.length, 1);
