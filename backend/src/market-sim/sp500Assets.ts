...
const _RAW_ASSETS: RawAsset[] = [
  ...
  },
  {
    ...
    volatility: 0.019,
    sector: "Health Care",
    dailyVolume: 5_500_000,
  },
  {
    symbol: "AVGO",
    initialPrice: 1342.00,
    volatility: 0.030,
    sector: "Technology",
    dailyVolume: 2_200_000,
  },
  {
    symbol: "COST",
    initialPrice: 785.20,
...
function deriveMarketCapB(price: number, dailyVolume: number): number {
  ...
  return parseFloat(
    ((price * sharesOutstandingM * 1_000_000) / 1e9).toFixed(1),
  );
}
...
function enrichAsset(
  ...
  return {
    ...
    marketCapB: curated?.marketCapB ??
      deriveMarketCapB(raw.initialPrice, raw.dailyVolume),
    beta: curated?.beta ?? deriveBeta(raw.volatility),
    dividendYield: curated?.dividendYield ??
...
export const ASSET_MAP = new Map<string, AssetDef>(
  SP500_ASSETS.map((a) => [a.symbol, a]),
);
