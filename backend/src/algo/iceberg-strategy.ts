...
await createTypedConsumer("iceberg-algo-routed", [
  {
    ...
    handler: (order: RoutedOrder) => {
      ...
      logger.info(
        `Queued ${order.orderId}: ${totalQty} ${order.asset} total, ${visibleQty} per slice (${Math.ceil(
          totalQty / visibleQty
        )} slices)`
      );
...
await createTypedConsumer("iceberg-algo-fills", [
  {
    topic: "orders.filled",
...
marketClient.onTick(async (tick) => {
  ...
  for (const order of [...activeOrders.values()]) {
    ...
    logger.info(
      `Slice ${order.sliceCount} for ${order.orderId}: ${order.currentSliceQty} ${order.asset} @ mkt ${marketPrice}`
    );
