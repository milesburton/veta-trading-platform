...
await createTypedConsumer("is-algo-routed", [
  {
    ...
    handler: (order: RoutedOrder) => {
      ...
      const urgency = Math.max(0.0, Math.min(1.0, Number(params.urgency ?? 0.5)));
      const maxSlippageBps = Math.max(1, Number(params.maxSlippageBps ?? 50));
      const minSlices = Math.max(1, Number(params.minSlices ?? 3));
      const maxSlices = Math.max(minSlices, Number(params.maxSlices ?? 10));
      ...
      logger.info(
        `Queued ${order.orderId}: ${order.quantity} ${order.asset} arrival=${arrivalPrice.toFixed(
          4
...
await createTypedConsumer("is-algo-fills", [
  {
    topic: "orders.filled",
    ...
    handler: (fill: FillEvent) => {
      ...
      if (order.totalRemaining <= 0) {
        ...
        logger.info(
          `Complete ${order.orderId}: filled=${order.filledQty} avg=${avgFill.toFixed(
            4
          )} slippage=${slipBps.toFixed(1)}bps (${isAdverse ? "adverse" : "favourable"})`
        );
  ...
  for (const order of [...activeOrders.values()]) {
    ...
    if (adverseDriftBps > order.maxSlippageBps) {
      if (!order.paused) {
        order.paused = true;
        logger.info(
          `Paused ${order.orderId}: drift=${adverseDriftBps.toFixed(
            1
    ...
    if (order.paused) {
      order.paused = false;
      logger.info(
        `Resumed ${order.orderId}: drift=${adverseDriftBps.toFixed(1)}bps now within threshold`
      );
      await producer
        ?.send("algo.heartbeat", {
          algo: ALGO,
    ...
    logger.info(
      `Slice ${order.sliceCount}/${order.numSlices} for ${order.orderId}: ${sliceQty} ${order.asset} @ limit ${childLimitPrice.toFixed(
        4
      )} (mkt ${marketPrice.toFixed(4)}, drift ${adverseDriftBps.toFixed(1)}bps)`
    );
