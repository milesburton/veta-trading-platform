...
export function buildPrompt(
  ...
  const topFactors = [...signal.factors]
    ...
    .slice(0, 2)
    .map((f) => `${f.name}(${f.contribution >= 0 ? "+" : ""}${f.contribution.toFixed(3)})`)
    .join(", ");

  const lines: string[] = [
    `Symbol: ${symbol}`,
    `Signal: ${signal.direction} | score ${signal.score.toFixed(3)} | confidence ${(signal.confidence * 100).toFixed(0)}%`,
    `Top factors: ${topFactors}`,
  ];

  if (fv) {
    lines.push(
      `Features: momentum=${fv.momentum.toFixed(4)}, relVol=${fv.relativeVolume.toFixed(2)}, ` +
      `realisedVol=${fv.realisedVol.toFixed(4)}, sectorRS=${fv.sectorRelativeStrength.toFixed(4)}, ` +
      `eventScore=${fv.eventScore.toFixed(2)}, newsVel=${fv.newsVelocity.toFixed(1)}, ` +
  ...
  if (rec) {
    lines.push(
      `Recommendation: ${rec.action} | qty ${rec.suggestedQty} | confidence ${(rec.confidence * 100).toFixed(0)}%`,
      `Rationale: ${rec.rationale}`,
    );
  }

  if (recentCloses.length > 0) {
    lines.push(`Recent closes: ${recentCloses.slice(-5).map((v) => v.toFixed(2)).join(", ")}`);
  }
