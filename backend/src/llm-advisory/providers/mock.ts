...
export function createMockProvider(): ILlmProvider {
  return {
    ...
    async generate(prompt: string, _systemPrompt: string): Promise<LlmProviderResponse> {
      await new Promise<void>((r) => setTimeout(r, 50));
      const encoder = new TextEncoder();
      const hash = await crypto.subtle.digest("SHA-256", encoder.encode(prompt));
      const index = new Uint8Array(hash)[0] % CANNED_RESPONSES.length;
      const text = CANNED_RESPONSES[index];
      return { text, promptTokens: 50, completionTokens: 120, latencyMs: 50, rawResponse: text };
    },
