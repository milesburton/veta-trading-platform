...
async function executeRfq(rfq: RfqRecord, quote: DealerQuote): Promise<void> {
  ...
  logger.info(
    `Execute ${execId}: ${rfq.side} ${rfq.quantity} ${rfq.asset} @ yield=${quote.yield.toFixed(
      4
...
async function processRfq(req: RfqRequest): Promise<void> {
  ...
  logger.info(
    `New RFQ ${rfqId}: ${req.side} ${req.quantity} ${req.asset} @ yield=${req.bondSpec.yieldAtOrder.toFixed(
      4
    )}`
  );
  ...
  await Promise.all([Promise.allSettled(quotePromises), sleep(QUOTE_WINDOW_MS)]);
...
Deno.serve({ port: PORT }, async (req) => {
  ...
  if (path === "/rfq/sellside" && req.method === "POST") {
    ...
    if (!clientUserId || !asset || !side || !quantity) {
      return jsonError("Missing required fields: clientUserId, asset, side, quantity", 400);
    }
    const now = Date.now();
  ...
  if (path === "/rfq/sellside/stats" && req.method === "GET") {
    const byState = [...sellSideRfqStore.values()].reduce<Record<string, number>>((acc, r) => {
      acc[r.state] = (acc[r.state] ?? 0) + 1;
      return acc;
    }, {});
  ...
  if (path === "/rfq/sellside" && req.method === "GET") {
    ...
    if (userId) {
      records = records.filter((r) => r.clientUserId === userId || r.salesUserId === userId);
    }
